MassiveOfQuestions = ['Какой язык программирования применяется для работы с базами данных MySQL?', "Выберите язык, который вы знайте", "Чем отличается функция от метода в Python?", "Как проверить, что две переменные ссылаются на один и тот же объект?",
...
"С помощью документо-ориентированных библиотек", "Что такое полиморфизм?", "Что такое инкапсуляция?", "Что такое абстрактный класс?", "Что такое интерфейс?", "Что означает аббревиатура REST?", "Какие операции HTTP используются в RESTful API?", "Что такое ресурс в RESTful API? ", "Какой формат данных обычно используется в RESTful API для передачи информации?", "Что означает код ответа HTTP 404 в RESTful API?", "Что означает аббревиатура VCS?", "Какая команда используется для создания новой ветки в git?", "3.	Какие команды используются для отправки изменений из локального репозитория в удаленный?", "4.	Какая команда используется для объединения двух веток в git?", "5.	Что произойдет, если в удаленный репозиторий были добавлены изменения, которых нет в локальной версии, и была использована команда git pull?"]

MassiveOfAnswers = [['SQL', 'Java', 'Python', 'Ruby', 'PHP'],["Python", "Java", "Php", "C#", "Ruby"],['A', 'B', 'C', 'D', "E"], ['Java', 'B', 'C', 'D', "E"],
...
const furtherQuestion = () =>{
    for(let i = 0; i < listAnswer.length; i++){
        if(listAnswer[i].checked == true){
            ...
            console.log(scores);
            if(step < 9){ //до это момента стоят чекбоксы
                step = step + 1;
            }
            else{
                showResult();
